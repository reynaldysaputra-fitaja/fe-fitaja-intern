      ...
      {cart.length === 0 ? (
        ...
        <div className="w-2/3">
          {cart.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between border-1 my-2 py-3"
            >
              <img src={item.image} alt={item.name} className="w-60 m-5 rounded-lg object-cover" />
                <div className="flex flex-col flex-grow mx-5">
                  <span className="font-bold">{item.name}</span>
                  <span className="text-sm font-bold">{item.discountprice || item.price}</span>
                  <span className="text-sm">Size: {item.size}</span>
                </div>
                <div className="flex flex-col m-5 gap-10 items-center">
                  <button
                  ...
                  <div className="border border-gray-500 flex items-center gap-3 p-2">
                    <button
                  ...
                  <button className="cursor-pointer underline font-bold">SAVE FOR LATER</button>
                </div>
            </div>
          ))}
          </div>
          ...
          <div className="flex flex-col gap-2 mx-auto w-70 p-5">
            <p className="font-bold text-lg text-center">Summary</p>
            <div className="flex justify-between my-5"><p className="text-gray-500 text-sm">Tambahkan kode promo</p><button>+</button></div>
            ...
            <button className="cursor-pointer bg-[#59F151] p-2 text-center">Lanjutkan ke Pembayaran</button>
          </div>
          <div>
            <p className="text-xl font-bold mt-10">RELATED PRODUCT</p>
            <Card limit={3}/>
          </div>
        </div>
      )}
